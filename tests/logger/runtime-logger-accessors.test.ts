...
describe("RuntimeLogger metadata accessors", () => {
  ...
  it(
    "does not execute metadata getters while redacting in-memory entries",
    () => {
  ...
  it(
    "does not execute nested or array accessors before console emission",
    () => {
      ...
      expect(() =>
        logger.info("nested accessors", { nested, items })
      ).not.toThrow();
      expect(getterReads).toBe(0);
